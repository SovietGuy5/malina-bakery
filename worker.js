...
export default { async fetch(request, env) { try { if(request.method==='OPTIONS') return new Response(null,{headers:cors}); const u=new URL(request.url); if(u.pathname==='/health') return json({ok:true}); if(request.method==='POST'&&u.pathname==='/order') return order(request,env); if(request.method==='POST'&&u.pathname==='/telegram-webhook') return webhook(request,env); return json({error:'Not found'},404); } catch(e) { return json({error:e.message||'Server error'},500); } } };
